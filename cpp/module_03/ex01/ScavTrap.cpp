...
void ScavTrap::attack( std::string const & target )
{
	if (_hit > 0)
	{
		if (_energy > 0)
		{
			std::cout << ANSI_COLOR_CYAN << "ScavTrap " << ANSI_COLOR_RESET << _name << " attacks " << ANSI_COLOR_RED  << target << ANSI_COLOR_RESET << ", causing " << ANSI_COLOR_RED  << _damage << ANSI_COLOR_RESET << " point of domage!" << std::endl;
			_energy--;
		}
		else
			std::cout << ANSI_COLOR_CYAN << "ScavTrap " << ANSI_COLOR_RESET << _name << " can't attack, no more energy" << std::endl;

	}
	else
		std::cout << ANSI_COLOR_CYAN << "ScavTrap " << ANSI_COLOR_RESET << _name << " can't attack, is dead" << std::endl;
